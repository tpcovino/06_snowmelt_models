...
neededPackages <- c("tidyverse", "lubridate", "units", "snotelr", "measurements", "tictoc", "progress", 'ggiraph', 'plotly', 'grid', 'tidyr') #, "extrafont")
...
sntl <- df 
sntl <- sntl %>%
  # Calculate daily changes in SWE and cumulative P
  mutate(SWEdiff.mm = swe.mm - lag(swe.mm, default = first(swe.mm)),
         Pdiff.mm = pcumul.mm - lag(pcumul.mm, default = first(pcumul.mm))) %>%
  # Replace negative Pdiff.mm values with 0
  mutate(Pdiff.mm = if_else(Pdiff.mm < 0, 0, Pdiff.mm)) %>%
  # Initialize input.mm as 0
  mutate(input.mm = 0) %>%
  # Calculate liquid input
  mutate(input.mm = case_when(
    Pdiff.mm == 0 & SWEdiff.mm < 0  ~ -SWEdiff.mm,
    Pdiff.mm > 0 & SWEdiff.mm < 0   ~ Pdiff.mm - SWEdiff.mm,
    Pdiff.mm > 0 & SWEdiff.mm == 0  ~ Pdiff.mm,
    TRUE ~ input.mm  # Preserve previous value
  )) %>%
  # Replace missing temperature values with 0
  mutate(temp_mean.c = replace_na(temp_mean.c, 0)) %>%
  # Prevent liquid input when temperature is below 0
  mutate(input.mm = if_else(temp_mean.c < 0, 0, input.mm)) %>%
