...
**Conceptual Models:** 
Conceptual models simplify the snowmelt process by establishing a simple, rule-based relationship between snowmelt and temperature. These models use a basic formula based on temperature to estimate how much snow will melt.
...
**Q1: (2 points) Using this map, what differences can you see between these two sites or what variation do you see in the watershed that may affect snow accumulation and melt?**
...
df <- snotel_download(site_id = 1186, internal = TRUE) %>%
    ...
    BLANK(date, swe.mm, precip.mm, temp_mean.c) %>%
    BLANK(wtr_yr = if_else(month(date) < 10, year(date), year(date) + 1)) %>%
    BLANK(wtr_yr) %>% # add a water year column
...
sntl <- sntl %>%
  ...
  mutate(input.mm = if_else(Pdiff.mm > 0 & SWEdiff.mm == 0 , 0, 0), # This is just insurance that input will always have a 0 in the row if other conditions aren't met. The condition doesn't really matter, the result is always 0
         input.mm = if_else(Pdiff.mm == 0 & SWEdiff.mm < 0, -SWEdiff.mm, input.mm), #1
         input.mm = if_else(Pdiff.mm > 0 & SWEdiff.mm < 0, Pdiff.mm - SWEdiff.mm, input.mm), #2
         input.mm = if_else(Pdiff.mm > 0 & SWEdiff.mm == 0, Pdiff.mm, input.mm)) %>% #3
...
**Q2: (3pts) In plain language, explain each step if-else statements above labeled #1, #2 and #3 (~3 brief sentences).**

ANSWER:

```{r}
# Create a cumulative input column from the daily data for visualization and calculations
sntl <- sntl %>%
  BLANK(wtr_yr) %>%
...
Observe differences in total observed SWE and total observed precipitation for the water year.
...
**Q3.(4 pnts) What processes could explain our calculated differences between cumulative input (calculated from SWE data) and cumulative precipitation, both measured at the same SNOTEL station? (List 1 or 2 processes). If the cumulative SWE input is found to be less than the cumulative precipitation at the same station, what processes could account for this discrepancy? (Provide at least 2 possible processes). Answer in 2-3 sentences.**
...
```{r}
# Import temperature data, mutate the date column to a date format, select the desired columns and filter by water year.
fc_precip <- read.csv("LFCmet_WY21_WY22.csv") %>%
  BLANK(date = lubridate::mdy_hm(TS)) %>%
  BLANK(date, water_year, PPT_10min..mm.) %>%
  BLANK(water_year %in% c(2021, 2022))
```
...
Even though the outlet meteorological station is fairly close to the SNOTEL station, the sites differ in elevation. Lets compare the observed cumulative precipitation between the upper watershed and the lower.
...
Run another set of simulations and compare the values of the best performing parameters between the upper and lower elevation sites. 
...
**Q4: (2pnts) Based on the differences in precipitation and temperature totals between the two sites, are SWE simulation results as you expected? (1 sentence)**

ANSWER: 


**Q5: (2pnts) How does the estimated threshold temperature distinguishing rain from snow change with elevation? Is the relationship between your parameter estimations as expected? How does this influence your confidence in the model? ** 

ANSWER

**Q6: (3 points) Compare the precipitation and SWE accumulation patterns between two sites. Discuss at least 3 potential natural and/or anthropogenic mechanisms driving the observed variations in precipitation and snow accumulation. Consider the factors you observed in question 1(2-3 sentences).**

ANSWER:

**Q7: (4 points) Let's say you want to model runoff for the Fool Creek catchment identified above. To do that, you want to estimate the timing and volume of snowmelt for this watershed. Refer to at least 2 model types reviewed in the 06_model_classifications lecture or terms in the lecture above and qualitatively describe a model that you could use to capture the spatial heterogeneity of snow accumulation in this watershed  (2-4 sentences).**

ANSWER:

**EXTRA CREDIT (2 points) Describe the process by which we arrived at different estimates for the threshold temperature distinguishing rain from snow at the upper and lower Fool Creek sites. In your explanation, describe how the 100 SWE simulations were used, how the best-performing run was selected, and how the final threshold temperature estimates were determined**
